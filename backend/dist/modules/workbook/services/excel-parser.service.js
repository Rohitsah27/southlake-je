...
let ExcelParserService = class ExcelParserService {
    ...
    async parseStarlightWorkbook(workbook, filename, forceOverwrite = false) {
        ...
        if (!headerInfo || pwRow === -1) {
            throw new common_1.BadRequestException('Invalid Starlight summary sheet layout');
        }
        ...
        else if ((b4Val.includes('al') || fnLower.includes('al')) &&
            (b4Val.includes('drp') || b4Val.includes('dpr') || fnLower.includes('drp') || fnLower.includes('dpr'))) {
            program = 'DPR AL';
        }
        else if ((b4Val.includes('mtc') || fnLower.includes('mtc')) &&
            (b4Val.includes('drp') || b4Val.includes('dpr') || fnLower.includes('drp') || fnLower.includes('dpr'))) {
            program = 'DRP MTC';
        }
    ...
    async parseFUTWorkbook(workbook, filename, forceOverwrite = false) {
        ...
        if (!reportDate) {
            throw new common_1.BadRequestException('Could not detect reporting date in FUT workbook');
        }
        ...
        if (stateExhibits.length === 0) {
            throw new common_1.BadRequestException('No state sheets (MTHLY-XX) found in the FUT workbook');
        }
        await this.stateExhibitRepo.save(stateExhibits);
    ...
    findRowIndexByLabel(sheet, labelSubstr) {
        const ref = sheet['!ref'];
        if (!ref)
            return -1;
    ...
    findRowIndexByExactLabel(sheet, labelStr) {
        const ref = sheet['!ref'];
        if (!ref)
            return -1;
        const range = XLSX.utils.decode_range(ref);
    ...
    getNumericValue(sheet, r, c) {
        if (r < 0 || c === null || c === undefined)
            return 0.00;
        const cell = sheet[XLSX.utils.encode_cell({ r, c })];
        if (!cell)
            return 0.00;
    ...
    getFUTReportDate(workbook) {
        const stateSheetName = workbook.SheetNames.find((name) => name.trim().match(/^MTHLY-([A-Z]{2})$/i) && name.trim() !== 'MTHLY-TOTAL');
        if (stateSheetName) {
    ...
    async validateSequentialMonth(program, monthKey, source) {
        ...
        if (!prevWb) {
            ...
            if (prevMonthKey === '2025-12') {
                throw new common_1.BadRequestException(`Validation Error: Baseline data for December 2025 is missing. ` +
                    `Please generate the ITD file from a Southlake file first and upload it using 'Upload Excel Workbook' to seed the database.`);
            }
            else {
                throw new common_1.BadRequestException(`Validation Error: Gaps in reporting month sequence are not allowed. ` +
    ...
    async generateITDWorkbookFromStarlight(fileBuffer) {
        const workbook = XLSX.read(fileBuffer, { type: 'buffer' });
        const sheetNames = workbook.SheetNames;
        const summarySheetName = sheetNames.find((name) => name === 'Starlight Excess' || name === 'Starlight APD') ||
            sheetNames.find((name) => name.toLowerCase().includes('starlight')) ||
        ...
        if (!summarySheet) {
            throw new common_1.BadRequestException('Invalid Starlight summary sheet');
        }
    ...
    createITDStateSheet(sheetName, itdValues) {
        const data = [];
        for (let i = 0; i < 13; i++) {
            data.push([]);
        }
        ...
        FIELDS.forEach(fieldId => {
            ...
            const label = FIELDS_LABELS[fieldId];
